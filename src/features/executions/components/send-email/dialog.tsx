...
export const SendEmailDialog = ({ open, onOpenChange, onSubmit, defaultValues = {} }: Props) => {
  const { data: credentials, isLoading } = useCredentialsByType(CredentialType.RESEND_API_KEY);
          ...
          <form onSubmit={form.handleSubmit((v) => { onSubmit(v); onOpenChange(false); })} className="space-y-6 mt-4">
            <FormField control={form.control} name="variableName" render={({ field }) => (
              <FormItem>
                ...
                <FormDescription>Reference as {`{{${varName}.emailId}}`}</FormDescription>
                <FormMessage />
              </FormItem>
            )} />
            ...
            <FormField control={form.control} name="credentialId" render={({ field }) => (
              <FormItem>
                ...
                <FormDescription>Get your API key at resend.com/api-keys.</FormDescription>
                <FormMessage />
              </FormItem>
            )} />
            ...
            <FormField control={form.control} name="from" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input placeholder="team@yourdomain.com" {...field} /></FormControl>
                <FormDescription>Defaults to noreply@resend.dev on free plan.</FormDescription>
                <FormMessage />
              </FormItem>
            )} />
            ...
            <FormField control={form.control} name="subject" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input placeholder="Your report is ready: {{myOpenAi.text}}" {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            ...
            <FormField control={form.control} name="body" render={({ field }) => (
              <FormItem>
                ...
                <FormDescription>HTML is supported. Use {`{{variables}}`} for dynamic content.</FormDescription>
                <FormMessage />
              </FormItem>
            )} />
