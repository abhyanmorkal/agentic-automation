...
function flattenObject(
  ...
  for (const [k, v] of Object.entries(obj)) {
    ...
    if (typeof v === "string" || typeof v === "number" || typeof v === "boolean") {
      ...
      fields.push(...flattenObject(v as Record<string, unknown>, path, depth + 1));
    }
  }
  return fields;
}
...
export const GoogleSheetsNode = memo((props: NodeProps<GoogleSheetsNodeType>) => {
  ...
  const upstreamSources = useMemo((): UpstreamSource[] => {
    ...
    function walk(targetId: string, depth: number) {
      ...
      for (const edge of inbound) {
        ...
        const varKey = NODE_TYPE_VAR_KEY[nodeType] ?? nodeType.toLowerCase().replace(/_/g, "");
        const label = NODE_TYPE_LABELS[nodeType] ?? nodeType.replace(/_/g, " ");
        ...
        const saved = nodeData?.savedResponses as Record<string, unknown> | undefined;
        let savedResponseNames = saved ? Object.keys(saved) : [];

        // Facebook Lead trigger stores data differently
        if (savedResponseNames.length === 0 && nodeType === "FACEBOOK_LEAD_TRIGGER") {
          const hasSample = nodeData?.sampleResponseSimple || nodeData?.sampleResponseAdvanced;
          if (hasSample) {
            // Synthesize virtual "response" entries that match what the dialog expects
            savedResponseNames = [];
            if (nodeData?.sampleResponseSimple) savedResponseNames.push("Simple");
            if (nodeData?.sampleResponseAdvanced) savedResponseNames.push("Advanced");
          }
        }

        // Use the node name or try a description
        const nodeName = (nodeData?.description as string) || (nodeData?.pageName as string) || "";
  ...
  const { savedResponseFields, activeResponseName, allResponseNames } = useMemo(() => {
    const empty = {
    ...
    const sourceNode = upstreamSources.find((s) => s.variableKey === selectedSource)
      ?? upstreamSources[0];

    if (!sourceNode) return empty;

    const srcNodeData = nodes.find((n) => n.id === sourceNode.nodeId)?.data as Record<string, unknown> | undefined;
    ...
    if (!saved && sourceNode.nodeType === "FACEBOOK_LEAD_TRIGGER") {
      ...
      const preferredName = userSelected && names.includes(userSelected)
        ? userSelected
        : names[0]!;

      // Map virtual response names to actual data properties
      const dataKey = preferredName === "Simple"
        ? "sampleResponseSimple"
        : "sampleResponseAdvanced";
      const data = srcNodeData?.[dataKey] as Record<string, unknown> | undefined;

      if (!data || typeof data !== "object") {
        return { savedResponseFields: [], activeResponseName: preferredName, allResponseNames: names };
      }

      const fields = flattenObject(data);
      return { savedResponseFields: fields, activeResponseName: preferredName, allResponseNames: names };
    }
    ...
    return { savedResponseFields: fields, activeResponseName: preferredName, allResponseNames: names };
  }, [props.data?.sourceVariable, props.data?.selectedResponseName, upstreamSources, nodes]);
